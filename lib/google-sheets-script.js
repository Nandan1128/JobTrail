...
function doPost(e) {
  try {
    ...
    if (action === 'addJob') {
      ...
      // Check if job already exists in sheet by ID, URL, or Title + Company
      const existingRow = findRowByJobId(sheet, job);
      if (existingRow > 0) {
    ...
    if (action === 'updateJob') {
      ...
      const target = job || data.id || data.url;

      const rowIndex = findRowByJobId(sheet, target);
      if (rowIndex > 0) {
        if (job) {
    ...
    if (action === 'syncAll') {
      ...
      for (const job of jobs) {
        const row = findRowByJobId(sheet, job);
        if (row > 0) {
